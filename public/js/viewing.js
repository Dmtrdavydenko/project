...
(async () => {
    ...
    const schedule = task[0].map(item => ({ time: item.time_milliseconds, name: `${item.type} ${item.yarn_name} ${item.density} ${item.additive_name === "нет" ? item.color : item.additive_name}` }));
