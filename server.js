...
async function getTime() {
    try {
        ...
        try {
            console.log('Успешно подключено к базе данных MySQL!');
            const sql = `SELECT timestamps.id, UNIX_TIMESTAMP(task_time) AS time_seconds, UNIX_TIMESTAMP(task_time) * 1000 AS time_milliseconds, tape.thread_density, tape.color, tape.additive_name
            FROM timestamps
            JOIN TapeExtrusion as tape ON timestamps.TapeExtrusion_id = tape.id
