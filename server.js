...
async function getTape() {
    try {
        ...
        try {
            console.log('Успешно подключено к базе данных MySQL!');
            const sql = "SELECT thread_density, color, thread_time * 60 as time_seconds, thread_time * 60 * 1000 as time_milliseconds " +
                "FROM TapeExtrusion " +
                "JOIN Thread_Parameters ON TapeExtrusion.thread_id = Thread_Parameters.thread_id " +
                "JOIN color ON TapeExtrusion.color_id = color.color_id " +
                "JOIN additive ON TapeExtrusion.additive_id = additive.additive_id " +
...
async function getTime() {
    try {
        ...
        try {
            console.log('Успешно подключено к базе данных MySQL!');
            const sql = `SELECT id, UNIX_TIMESTAMP(task_time) AS time_seconds, UNIX_TIMESTAMP(task_time) * 1000 AS time_milliseconds FROM timestamps 
